...
class RetrieveUpdateEvent(RetrieveUpdateAPIView):
    ...
    def get_queryset(self):
        ...
        in_support_team = get_user_model().objects.filter(pk=user.id, groups__name='support').exists()
        in_management_team = get_user_model().objects.filter(pk=user.id, groups__name='management').exists()
